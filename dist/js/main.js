...
;(function(window) {
    ...
    function LinearGradients(q, options){
        this.q = q;
    ...
    LinearGradients.prototype._init = function(){
    ...
    function createGradient(bg){
        var gradient = [];
        gradient.push('linear-gradient(to bottom');
        for (var i = 0; i < draggers.length; i++) {
            gradient.push(draggers[i].colour +' ' + draggers[i].deep);
        }
        if(bg !== undefined){
            gradient.push(bg+' 100%)');
        } else {
            gradient.push('#000 100%, #FFF 100%)');    
        }
    ...
    // to global 
    window.LinearGradients = LinearGradients;
    window.init = init;
